...
static int FLAGS_num = 5000*10000;
...
static uint64_t FLAGS_pmem_size = 0;
static uint64_t FLAGS_pmem_size_factor = 0;
  ...
  : cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size) : nullptr),
    ...
    if (!FLAGS_use_existing_db) {
      DestroyDB(FLAGS_db, Options());
		...
		//PMemsize = (DBImpl::options.pmem_size_ >> 20);
  }
  ...
  void Run() {
    ...
    while (benchmarks != nullptr) {
      ...
      if (name == Slice("open")) {
        ...
        HeapProfile();
      } else if (name == Slice("loadworkloada")) {
				fresh_db = true;
        std::string trace_name = FLAGS_trace + "/loada.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
			} else if (name == Slice("loadworkloade")) {
				fresh_db = true;
				std::string trace_name = FLAGS_trace + "/loade.csv";
				LoadTrace(trace_name);
				method = &Benchmark::RunTrace;
      } else if (name == Slice("workloada")) {
        std::string trace_name = FLAGS_trace + "/worka.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
      } else if (name == Slice("workloadb")) {
        std::string trace_name = FLAGS_trace + "/workb.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
      } else if (name == Slice("workloadc")) {
        std::string trace_name = FLAGS_trace + "/workc.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
      } else if (name == Slice("workloadd")) {
        std::string trace_name = FLAGS_trace + "/workd.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
      } else if (name == Slice("workloade")) {
        std::string trace_name = FLAGS_trace + "/worke.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
      } else if (name == Slice("workloadf")) {
        std::string trace_name = FLAGS_trace + "/workf.csv";
        LoadTrace(trace_name);
        method = &Benchmark::RunTrace;
 ...
 private:
  ...
  void Open() {
    ...
    options.max_file_size = FLAGS_max_file_size;
		options.pmem_size = FLAGS_pmem_size;
		options.pmem_size_factor = FLAGS_pmem_size_factor;
  ...
  void ReadWhileWriting(ThreadState* thread) {
    ...
    for (const auto& operation : ycsb_trace) {
      ...
      if (operation.operation_type == 'i') {
        char key[100];
        snprintf(key, sizeof(key), "%023lu", operation.key);
...
int main(int argc, char** argv) {
  ...
  FLAGS_block_size = leveldb::Options().block_size;
	FLAGS_pmem_size = leveldb::Options().pmem_size;
	FLAGS_pmem_size_factor = leveldb::Options().pmem_size_factor;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_block_size = n;
		} else if (sscanf(argv[i], "--pmem_size=%d%c", &n, &junk) == 1) {
			FLAGS_pmem_size = n * 1024L * 1024L;
		} else if (sscanf(argv[i], "--pmem_size_factor=%d%c", &n, &junk) == 1) {
			FLAGS_pmem_size_factor = n;
