...
TOID(char) PSkipList<Key,Comparator>::NewNode(TOID(char) key, int height) {
  ...
  TOID(char) mem = D_RW(pmemarena_)->AllocateAligned(bytes);
	//Node* tmp = new Node(key, height);
	//memcpy(D_RW(mem), tmp, sizeof(tmp));

	const char* tmp = D_RO(key);
...
inline TOID(char) PSkipList<Key,Comparator>::Iterator::key() const {
  assert(Valid());
  //return D_RO(((Node*)D_RO(node_))->key);
...
inline void PSkipList<Key,Comparator>::Iterator::SeekToFirst() {
  node_ = ((Node*)D_RO(list_->head_))->Next(0);
}
	...
	const 
{
	...
	while(true)
	{
		TOID(char) next = ((Node*)D_RW(x))->Next(level);
...
void PSkipList<Key,Comparator>::Merge(TOID(char) target, TOID(char)* prev)
{
	...
	for(int i = 0; i < min_height; i++)
	{
		...
		if(i == 0)
			pmemobj_persist(pop, ((Node*)D_RW(target_))->GetBase(), 16);
		((Node*)D_RW(prev_[i]))->NoBarrier_SetNext(i, target_);
		if(i == 0)
			pmemobj_persist(pop, ((Node*)D_RW(prev_[i]))->GetBase(),16);
	}
