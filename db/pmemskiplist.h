...
class PSkipList {
 private:
  struct Node;
	bool main_node;
	bool bool_merge[config::kSkiplistMaxHeight];
...
TOID(char) PSkipList<Key,Comparator>::NewNode(TOID(char) key, int height) {
	...
	unsigned int dummy = sizeof(Node) + 8 * (height - 1);
	unsigned int bytes = dummy + ((64 - dummy%64)%64);
...
inline void PSkipList<Key,Comparator>::Iterator::Next() {
  assert(Valid());
	node_.oid.off = ((Node*)D_RW(node_))->Next(0);
}
...
inline void PSkipList<Key,Comparator>::Iterator::SeekToFirst() {
	node_ = list_->head_;
	node_.oid.off = ((Node*)D_RO(list_->head_))->Next(0);
}
...
template<typename Key, class Comparator>
inline int PSkipList<Key,Comparator>::Iterator::GetHeight()
{
	return ((Node*)D_RO(node_))->height;
}
...
bool PSkipList<Key,Comparator>::KeyIsAfterNode(const Key& key, TOID(char) n) const {
  return (n.oid.off != 0) && (compare_(D_RO(((Node*)D_RW(n))->key), key) < 0);
}
	...
	const 
{
	...
	TOID(char) next;
	next.oid.pool_uuid_lo = x.oid.pool_uuid_lo;

	while(true)
	{
		next.oid.off = ((Node*)D_RW(x))->Next(level);
		if (KeyIsAfterNode(key, next))
		{
			x.oid.off = next.oid.off;
		}
		else
		{
			prev[level] = x;
...
PSkipList<Key,Comparator>::FindLessThan(const Key& key) const {
  ...
  while (true) {
    ...
    next.oid.off = ((Node*)D_RW(x))->Next(level);
    if ((next.oid.off == 0) || compare_(D_RO(((Node*)D_RW(next))->key), key) >= 0) {
      if (level == 0) {
        return x;
      } else {
        // Switch to next list
...
template<typename Key, class Comparator>
void PSkipList<Key,Comparator>::Merge(TOID(char) target, TOID(char)* prev, bool* prev_merge)
{
	TOID(char) target_ = target;
	TOID(char)* t_prev = prev;
	bool* prev_merge_ = prev_merge;
	...
	for(int i = 0; i < min_height; i++)
	{
		if(bool_merge[i])
		{
			((Node*)D_RW(target_))->NoBarrier_SetNext(i, ((Node*)D_RW(t_prev[i]))->NoBarrier_Next(i));
		...
		if(prev_merge[i])
		{
			((Node*)D_RW(t_prev[i]))->NoBarrier_SetNext(i, target_.oid.off);


			if(i == 0)
				pmemobj_persist(pop, ((Node*)D_RW(t_prev[i]))->GetBase(),8);
		}
		bool_merge[i] = prev_merge[i];
	}
	if(min_height > GetMaxHeight())
	{
		max_height_.NoBarrier_Store(reinterpret_cast<void*>(min_height));
		D_RW(pmemarena_)->max_height_ = min_height;
	}
}
